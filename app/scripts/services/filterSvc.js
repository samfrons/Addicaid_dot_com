...
angular.module('addicaidSiteApp')
  .factory('filterSvc', ['$filter', function($filter) {
    ...
    // TODO: this needs to be put inside a scope, don't pollute the global scope!
    ...
    filterSvc.filtersToApply = function() {
      ...
      if (angular.isDefined(fellowshipFilters)) {
        filters.push(fellowshipFilters);
      }

      var dayFilters = $filter('selectedFiltersOr')(filterSvc.filters.days);
      if (angular.isDefined(dayFilters)) {
        filters.push(dayFilters);
      }
...
//            if (timeFilters.length > 0) filters.push(timeFilters);
      ...
      if (angular.isDefined(ratingFilters)) {
